...
class ProductSeeder extends BaseSeeder
{
    public function run(): void
    {
        ...
        foreach ($products as $key => $item) {
            ...
            for ($j = 0; $j < $faker->numberBetween(1, 5); $j++) {
                $variation = Product::create([
                    ...
                    'price' => $product->price,
                    'sale_price' => is_int($product->id) && $product->id % 4 == 0 ? ($product->price - $product->price * $faker->numberBetween(
                        10,
                        30
                    ) / 100) : null,
        ...
        foreach ($translations as $index => $item) {
            $item['lang_code'] = 'vi';
            $product = Product::skip($index)->first();
            $item['ec_products_id'] = $product->id;
