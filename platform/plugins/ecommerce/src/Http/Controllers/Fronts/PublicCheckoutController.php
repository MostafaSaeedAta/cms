...
use Botble\Ecommerce\Traits\TapTrait;
...
class PublicCheckoutController
{
    ...
    public function getCheckoutSuccess(string $token, BaseHttpResponse $response)
    {
        ...
        return view('plugins/ecommerce::orders.thanks', compact('order', 'products' ,'currencies'));
        // return view('plugins/ecommerce::orders.thank-you', compact('order', 'products' ,'currencies'));
    }
    ...
    public function paymentOrder(Request $request, BaseHttpResponse $response)
    {
        ...
        try {
            ...
            $request->merge([
                ...
                'status' => OrderStatusEnum::PENDING,
                'is_finished' => false,
                'token' => $request['checkout-token'],
            ]);
            // dd($request->all());
            $order = $this->orderRepository->getFirstBy(['token' => $request['checkout-token']]);
            ...
            // if ($order->status != OrderStatusEnum::COMPLETED|| $order->price == 0) {
            //     return $response
            //     ->setError()
            //     ->setNextUrl(PaymentHelper::getCancelURL())
            //     ->setMessage(__('Payment failed!'));;
            ...
            $payment =  Payment::create([
                ...
                "status" => $order->status,
                "payment_channel" => PaymentMethodEnum::TAP,
                "payment_date" => now(),
                "order_id" => $order->id,
            ...
            // $redirect_url = route("public.checkout.success", $order->token);
            $redirect_url = route('redirect_after_order', ['payment_id' => $payment->id]);

            $result =   $this->create_charge($payment, $request->user(), $redirect_url);
            ...
            if ($result["error"]) {
                ...
                $payment->update(["transaction_ref" => $result['data']->id, "payment_response" => json_encode($result['data']), "currency" => $result['data']->currency, "charge_id" => $result['data']->id]);
                return $response->setMessage($result["message"])
                    ->setNextUrl($result['data']->transaction->url);
            }
    ...
    public function redirect_after_order($id)
    {

        $payment = Payment::find($id);
        $result = $this->check_payment($payment->charge_id);
        if (isset($result['data']->status) && $result['data']->status === 'CAPTURED') {
            //success
            ...
            $payment->update(["paid" => $payment->amount, "payment_response" => json_encode($result['data']), "status" => PaymentStatusEnum::COMPLETED]);
            return redirect()->to(route('public.checkout.success', OrderHelper::getOrderSessionToken()));


            // return $response ->setMessage(__('Checkout successfully!'));

            // ->setNextUrl(PaymentHelper::getRedirectURL());

        }
        //error
        // return $this->sendError(isset($response["data"]->response->message)?$response["data"]->response->message:"لم تتم عملية الدفع بنجاح", null,500);
    }//end fuc
